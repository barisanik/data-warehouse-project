...
CREATE VIEW gold.fact_sales AS 
    ...
    FROM(
        ...
        FROM 
            ...
            JOIN gold.dim_products pr ON sd.sls_prd_key = pr.product_number
            JOIN gold.dim_customers cu ON sd.sls_cust_id = REPLACE(cu.customer_id,'CSV-','') AND cu.[data_source] = 'crm-csv'
        ...
        SELECT
            ...
            ,cu.[data_source]                   AS [data_source]
        FROM
            silver.djapi_order o
            JOIN gold.dim_customers cu ON o.cust_id = REPLACE(cu.customer_id,'API-','') AND cu.[data_source] = 'dummyjson-api'
            JOIN gold.dim_products pr ON o.prd_id = REPLACE(pr.product_id,'API-','') AND pr.[data_source] = 'dummyjson-api'
    ) a;
GO