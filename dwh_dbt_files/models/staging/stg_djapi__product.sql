...
cleaned AS(
    SELECT
        ...
        ,{{ fn_initcap("REPLACE(title, '\\r', '')") }} AS title -- Set first character of each word uppercase.
        ,INITCAP(REPLACE(IFNULL(REPLACE(category, '\r', ''),'n/a'),'-',' ')) AS category -- Set first character of each word uppercase.
        ,CASE 
